...
struct maru_fifo {
  ...
  /** Notification fd for writer side. Uses pipe. */
  int write_pipe[2];

  /** Notification pipes for reader side. Uses pipe. */
  int read_pipe[2];
...
void maru_fifo_free(maru_fifo *fifo) {
  ...
  if (fifo->read_pipe[0] >= 0)
    close(fifo->read_pipe[0]);
  close(fifo->read_pipe[1]);
  if (fifo->write_pipe[0] >= 0)
    close(fifo->write_pipe[0]);
  close(fifo->write_pipe[1]);
...
maru_fifo *maru_fifo_new(size_t size) {
  ...
  pipe(fifo->write_pipe);
  write(fifo->write_pipe[1], "1", 1);
  pipe(fifo->read_pipe);
  if (fifo->write_pipe[0] < 0 || fifo->read_pipe[0] < 0)
    goto error;
...
int maru_fifo_write_notify_fd(maru_fifo *fifo) {
  return fifo->write_pipe[0];
}

int maru_fifo_read_notify_fd(maru_fifo *fifo) {
  return fifo->read_pipe[0];
}
...
maru_error maru_fifo_write_unlock(maru_fifo *fifo,
  ...
  if (maru_fifo_read_avail_nolock(fifo) >= fifo->read_trigger && fifo->read_pipe[1] >= 0)
    write(fifo->read_pipe[1], "1", 1);
...
maru_error maru_fifo_read_unlock(maru_fifo *fifo,
  ...
  if (maru_fifo_write_avail_nolock(fifo) >= fifo->write_trigger && fifo->write_pipe[1] >= 0)
    write(fifo->write_pipe[1], "1", 1);
...
static inline void maru_fifo_read_notify_ack_nolock(maru_fifo *fifo) {
  ...
  if (maru_fifo_read_avail_nolock(fifo) < fifo->read_trigger) {
    uint64_t val;
    read(fifo->read_pipe[0], &val, sizeof(val));
  }
}
...
static inline void maru_fifo_write_notify_ack_nolock(maru_fifo *fifo) {
  ...
  if (maru_fifo_write_avail_nolock(fifo) < fifo->write_trigger) {
    uint64_t val;
    read(fifo->write_pipe[0], &val, sizeof(val));
  }
}
